...
if [[ $BRANCH == "extended" ]]; then
	...
	export VERSION="Kernel-Hmp-Oldcam-${STABLE_RELEASE_VERSION}"
	elif [[ "$@" =~ "newcam" ]]; then
	export VERSION="Kernel-Hmp-Newcam-${STABLE_RELEASE_VERSION}"
	fi
elif [[ $BRANCH == "extended-eas" ]]; then
    if [[ "$@" =~ "oldcam" ]]; then
	curl -s -X POST https://api.telegram.org/bot${BOT_API_TOKEN}/sendMessage -d text="<i><b>Extended Eas ${STABLE_RELEASE_VERSION} build's started on CI...</b></i>" -d chat_id=${KERNEL_CHAT_ID_PUBLIC} -d parse_mode=HTML
	export VERSION="Kernel-Eas-Oldcam-${STABLE_RELEASE_VERSION}"
	elif [[ "$@" =~ "newcam" ]]; then
	export VERSION="Kernel-Eas-Newcam-${STABLE_RELEASE_VERSION}"
	fi
else
    if [[ "$@" =~ "oldcam" ]]; then
	...
	export VERSION="Kernel-Beta-Eas-Oldcam-${BRANCH}-${SEMAPHORE_WORKFLOW_NUMBER}"
	elif [[ "$@" =~ "newcam" ]]; then
	export VERSION="Kernel-Beta-Eas-Newcam-${BRANCH}-${SEMAPHORE_WORKFLOW_NUMBER}"
	fi
fi
